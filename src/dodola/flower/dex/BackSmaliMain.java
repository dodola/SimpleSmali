...
public class BackSmaliMain {
    ...
    public static void main(String[] args) {
        ...
        //                classToFile(printStream, aClass);
        //            }
        //
        //        } catch (IOException e) {
        //            e.printStackTrace();
    ...
    private static void classToFile(final Writer printStream, DexBackedClassDef aClass) throws IOException {
        Indentation indent = new Indentation(0);

        printStream.write(classToString(aClass));
        ...
        for (DexBackedField field : fields) {
            ...
            for (DexBackedAnnotation annotation : annotations) {
                annotionToString(printStream, annotation, indent);
            }

            printStream.write(indent.toString());
            printStream.write(String
                    .format("%s %s %s", accessFlagsToString(field.getAccessFlags()), typeToString(field.getType()),
                            field.getName()));
            printStream.write(newLine);
        ...
        for (DexBackedMethod dexBackedMethod : methods) {
            ...
            for (Set<? extends DexBackedAnnotation> parameterAnnotationSet : parameterAnnotations) {
                for (DexBackedAnnotation annotation : parameterAnnotationSet) {
                    annotionToString(printStream, annotation, indent);
                }
            }
            ...
            printStream.write(methodToString(dexBackedMethod, localInfo));
            printStream.write(" {");
            printStream.write(newLine);
            ...
            if (methodImplementation != null) {
                ...
                for (BuilderInstruction builderInstruction : instructions) {
                    ...
                    if (targetsLabels != null) {
                        printStream.write(newLine);
                        printStream.write(String.format("%s:%s", indent.toString(), String.join(", ", targetsLabels)));
                        printStream.write(newLine);
    ...
    private static boolean disassembleClass(DexBackedClassDef classDef, ClassFileNameHandler fileNameHandler) {
        ...
        try {
            ...
            if (!smaliParent.exists()) {
                if (!smaliParent.mkdirs()) {
                    if (!smaliParent.exists()) {
                        System.err
                                .println("Unable to create directory " + smaliParent.toString() + " - skipping class");
                        return false;
                    }
                }
            }
            ...
            classToFile(writer, classDef);
        } catch (Exception ex) {
            System.err.println("\n\nError occurred while disassembling class " + classDescriptor.replace('/', '.')
                    + " - skipping class");
            ex.printStackTrace();
            smaliFile.delete();
            return false;
    ...
    public static boolean disassembleDexFile(DexBackedDexFile dexFile, File outputDir, int jobs) {
        List<? extends DexBackedClassDef> classDefs = Ordering.natural().sortedCopy(dexFile.getClasses());
        ...
        for (final DexBackedClassDef classDef : classDefs) {
            tasks.add(executor.submit(new Callable<Boolean>() {
                @Override public Boolean call() throws Exception {
                    return disassembleClass(classDef, fileNameHandler);
                }
            }));
        }
